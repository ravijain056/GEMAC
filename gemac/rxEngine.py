...
def rxengine(rxclientintf, rxgmii_intf, rxflowintf, rxconfig0, rxconfig1,
    ...
    def assign():
        ...
        calccrc.next = state == (rxstate.INFRAME or rxstate.PAUSE or rxstate.PASS)
        rxflowintf.macaddr.next = (rxconfig1[16:] << 32) | rxconfig0
        matchcrc.next = crcout == 0x2144DF1C

    @always_seq(rxgmii_intf.clk.posedge, reset)
    def curbyteinc():
        """ Index Incrementer """
        if state == rxstate.IDLE or rxstate.PREAMBLE:
            curbyte.next = 1
        else:
    ...
    def receiver():
        ...
        if rxen:
            if rxgmii_intf.err:
                ...
                gofilt.next = False
                if curbyte == 6:
                    if isucast1 or isucast2 or ismcast1 or ismcast2 or isbcast or ispause or filterconfig[31]:
                ...
                if curbyte == (len + 14 + 4*vlanen + 4 + 6):
                    rxclientintf.dv.next = False
    ...
    return assign, curbyteinc, receiver, crc32inst, dlinst, afinst1,\
        afinst2, afinst3, afinst4, afinst5, afinst6
